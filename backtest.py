# =============================================================================
# backtest.py — EURUSD Strategy Backtester (v5 — canonical clean version)
#
# Usage:
...
# =============================================================================
# STRATEGY PARAMETERS  — edit here, re-run, compare
# =============================================================================

SYMBOL            = "EURUSD"

# ── Instrument pip config ─────────────────────────────────────────────────
# EURUSD : PIP_SIZE = 0.0001,  PIP_VALUE_PER_LOT = 10.0  ($10/pip/lot)
# XAUUSD : PIP_SIZE = 0.01,    PIP_VALUE_PER_LOT = 1.0   ($1/pip/lot)
PIP_SIZE          = 0.0001
...
# ── Signal filters ────────────────────────────────────────────────────────
RSI_LONG_MIN      = 42       # RSI floor for longs
RSI_LONG_MAX      = 65       # RSI ceiling for longs
RSI_SHORT_MIN     = 35       # RSI floor for shorts
RSI_SHORT_MAX     = 58       # RSI ceiling for shorts
ADX_MIN           = 20       # H1 ADX minimum — 20 filters flat chop while keeping
                              # enough signals for statistical significance (~30/6mo)
EMA_SEP_PIPS      = 0.3      # Min pip separation on cross bar (removes false touches)

# ── Higher timeframe regime filter ────────────────────────────────────────
USE_H4_FILTER     = False    # H4 + ADX>25 was double-filtering — only 1-2 trades/month
                              # H1 EMA stack already provides the regime guard
                              # Set True to re-enable if you want to test it

# ── Trade sizing — 1:2 RR increases TP hit rate to ~45% ──────────────────
SL_ATR_MULT       = 1.5      # SL = ATR × this
REWARD_RATIO      = 2.0      # TP = SL × 2  →  1:2 RR, needs 33% win rate to break even
RISK_PER_TRADE    = 0.02     # 2% equity risk per trade (compounding)
...
# ── Daily limits ──────────────────────────────────────────────────────────
MAX_DAILY_LOSS    = 0.02     # Kill switch: suspend EA if daily loss hits -2%
MAX_OPEN_TRADES   = 3        # Max concurrent positions

# ── Costs ─────────────────────────────────────────────────────────────────
COMMISSION_USD    = 7.0      # Round-trip commission per standard lot
SPREAD_PIPS       = 1.5      # Typical EURUSD spread in pips
INITIAL_BALANCE   = 35.0
...
def fetch_mt5(start: datetime, end: datetime, days: int = 180) -> tuple:
    """Fetch H1, H4, M15 OHLCV from MT5. Returns (h1, h4, m15)."""
    try:
    ...
    def _get(tf, label, min_bars):
        ...
        if rates is None or len(rates) < min_bars:
            logger.warning(
                f"{label}: only {len(rates) if rates is not None else 0} bars "
                f"from date range — falling back to copy_rates_from_pos"
            )
            needed = {
                mt5.TIMEFRAME_H1:  24  * days,
                mt5.TIMEFRAME_H4:  6   * days,
                mt5.TIMEFRAME_M15: 96  * days,
            }.get(tf, days * 24)
        ...
        # Trim to requested window — prevents fallback returning years of extra data
    ...
    h4  = _get(mt5.TIMEFRAME_H4,  "H4",  min_bars=days * 4)
    m15 = _get(mt5.TIMEFRAME_M15, "M15", min_bars=days * 67)
    mt5.shutdown()
    return h1, h4, m15


def make_synthetic(days: int = 180) -> tuple:
    """Generate synthetic EURUSD data for offline testing. Returns (h1, h4, m15)."""
...
def generate_signals(h1_raw: pd.DataFrame,
                     h4_raw: pd.DataFrame,
                     m15_raw: pd.DataFrame) -> pd.DataFrame:
    """
    Three-timeframe signal engine: H4 regime -> H1 trend -> M15 entry.

    H4  : EMA50 vs EMA200 — defines macro bias (bull/bear)
    H1  : EMA stack + ADX — confirms trend strength and direction
    M15 : EMA9 crossover + RSI — fires the actual entry signal

    All three must agree for a trade to be taken.
    merge_asof aligns higher-TF values to each M15 bar cleanly.
    """
    ...
    h4_signals["h4_bull"] = h4["ema50"] > h4["ema200"]
    h4_signals["h4_bear"] = h4["ema50"] < h4["ema200"]
    ...
    merged = pd.merge_asof(
        ...
        h4_signals.reset_index().sort_values("time"),
    ...
    # ── Align H1 -> M15 ───────────────────────────────────────────────────
    merged = pd.merge_asof(
        merged.reset_index().sort_values("time"),
        h1_signals.reset_index().sort_values("time"),
        on="time", direction="backward",
    ).set_index("time")
    ...
    # ── EMA separation — same-bar check ───────────────────────────────────
    sep = EMA_SEP_PIPS * PIP_SIZE
    ...
    # ── Layered funnel debug ───────────────────────────────────────────────
    n = len(merged)
    rsi_long  = merged["rsi"].between(RSI_LONG_MIN,  RSI_LONG_MAX)
    rsi_short = merged["rsi"].between(RSI_SHORT_MIN, RSI_SHORT_MAX)

    l1 = merged["cross_up"];    l2 = l1 & merged["sep_ok_up"]
    l3 = l2 & merged["in_session"]; l4 = l3 & merged["h1_bull"]
    l5 = l4 & merged["h1_adx_ok"]; l6 = l5 & rsi_long
    l7 = l6 & h4_long_ok

    s1 = merged["cross_down"];  s2 = s1 & merged["sep_ok_down"]
    s3 = s2 & merged["in_session"]; s4 = s3 & merged["h1_bear"]
    s5 = s4 & merged["h1_adx_ok"]; s6 = s5 & rsi_short
    s7 = s6 & h4_short_ok
    ...
    logger.info(f"  + H4 aligned -> SIGS  : {l7.sum():,}  <- final long signals")
    ...
    logger.info(f"  + H4 aligned -> SIGS  : {s7.sum():,}  <- final short signals")
    logger.info("─────────────────────────────────────────────────────")
...
def simulate(bars: pd.DataFrame) -> dict:
    """
    Bar-by-bar simulation. Direct 1:3 RR.
    TP checked before SL — if both hit on same bar, TP wins.
    Lot size compounds with equity on every trade.
    All pip calculations use PIP_SIZE and PIP_VALUE_PER_LOT constants.
    """
    ...
    for i, row in rows.iterrows():
        ...
        for pos in open_pos:
            ...
            # TP priority: credit TP if both SL and TP hit on same bar
            tp_hit = (d ==  1 and hi >= pos["tp"]) or \
                     (d == -1 and lo <= pos["tp"])
            sl_hit = (d ==  1 and lo <= pos["sl"]) or \
                     (d == -1 and hi >= pos["sl"])
            ...
            pnl      = (pip_gain / PIP_SIZE * PIP_VALUE_PER_LOT * pos["lots"]
                        - COMMISSION_USD * pos["lots"])   # full round-trip
        ...
        if can_trade:
            ...
            if sl_pips > 0:
                # Compounding: risk% of CURRENT equity, auto-scales as balance grows
                risk_amount = equity * RISK_PER_TRADE
                raw_lots    = risk_amount / (sl_pips * PIP_VALUE_PER_LOT)
                lots        = round(raw_lots / 0.01) * 0.01
                lots        = max(0.01, min(lots, 100.0))

                balance -= COMMISSION_USD * lots * 0.5   # entry commission
...
def compute_stats(results: dict) -> dict:
    ...
    return {
        ...
        "lot_growth_pct":    round((trades[-1]["lots"] / trades[0]["lots"] - 1) * 100, 1)
...
def main():
    ...
    print(f"  Symbol          : {SYMBOL}  |  H4 filter: {'ON' if USE_H4_FILTER else 'OFF'}")
    print(f"  RR              : 1:{REWARD_RATIO:.0f}  |  Risk: {RISK_PER_TRADE*100:.1f}%  |  ADX>{ADX_MIN}")
    print("-" * 52)
    print(f"  Total trades    : {stats.get('total_trades', 0)}")
    print(f"  Win rate        : {stats.get('win_rate', 0):.1f}%  (breakeven: {100/(1+REWARD_RATIO):.0f}%)")
    ...
    print(f"  Lot growth      : {stats.get('lot_growth_pct', 0):+.1f}%  (compounding)")
    print("=" * 52)
