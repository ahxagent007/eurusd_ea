...
ADX_MIN           = 18       # Min H1 ADX — filters flat chop
EMA_SEP_PIPS      = 0.3      # Min pip separation on cross bar
...
# Costs
COMMISSION_USD    = 7.0      # Round-trip commission per lot
SPREAD_PIPS       = 1.5      # Typical EURUSD spread
...
def fetch_mt5(start: datetime, end: datetime, days: int = 180) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def _get(tf, label, min_bars):
        ...
        if rates is None or len(rates) < min_bars:
            ...
            needed = {mt5.TIMEFRAME_H1: 24 * days, mt5.TIMEFRAME_M15: 96 * days}.get(tf, days * 24)
...
def make_synthetic(days: int = 180) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    logger.info(f"Synthetic: {len(h1):,} H1 bars, {len(m15):,} M15 bars")
    return h1, m15
...
def generate_signals(h1_raw: pd.DataFrame, m15_raw: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    # For every M15 bar, pick the most recent H1 bar at or before it.
    # This is the correct way — no resample/reindex double-fill artifacts.
    h1_reset  = h1_signals.reset_index()    # columns: time, h1_bull, h1_bear, h1_adx_ok
    m15_reset = m15.reset_index()           # columns: time, close, ema9, ..., cross_up, ...
    ...
    # Fill any leading NaN (M15 bars before first H1 bar) with False
    ...
    logger.info(f"  + RSI zone → SIGNALS  : {l6.sum():,}  ← final long signals")
    ...
    logger.info(f"  + RSI zone → SIGNALS  : {s6.sum():,}  ← final short signals")
    logger.info("─────────────────────────────────────────────────────")
...
def simulate(bars: pd.DataFrame) -> dict:
    ...
    for i, row in rows.iterrows():
        ...
        for pos in open_pos:
            ...
            pnl      = pip_gain / 0.0001 * 10 * pos["lots"]                        - COMMISSION_USD * pos["lots"]   # full round-trip on close
        ...
        if not day_killed and daily_pnl_pct <= -MAX_DAILY_LOSS:
            ...
            for pos in open_pos:
                ...
                pnl      = pip_gain / 0.0001 * 10 * pos["lots"] - COMMISSION_USD * pos["lots"] * 0.5
                balance += pnl
                equity  += pnl
                trades.append({
                    "entry_time":    rows.at[pos["entry_i"], "time"].strftime("%Y-%m-%d %H:%M"),
        ...
        if can_trade:
            ...
            if sl_pips > 0:
                ...
                pip_value   = 10.0          # $10 per pip per standard lot (EURUSD/USD acct)
        ...
        # ── Equity curve ───────────────────────────────────────────────────
        floating = sum(
            (cl - p["entry"]) * p["dir"] / 0.0001 * 10 * p["lots"]
            for p in open_pos
        )
...
def main():
    ...
    if args.no_mt5:
        h1, m15 = make_synthetic(days=args.days)
    else:
        h1, m15 = fetch_mt5(start, end, days=args.days)

    logger.info("Generating signals...")
    signals = generate_signals(h1, m15)
